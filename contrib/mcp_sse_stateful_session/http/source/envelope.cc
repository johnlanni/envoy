...
EnvelopeSessionStateFactory::parseAddress(Envoy::Http::RequestHeaderMap& headers) const {
  ...
  const auto params = Envoy::Http::Utility::QueryParamsMulti::parseQueryString(path->value().getStringView());
  auto session_value_opt = params.getFirstValue(param_name_);
  if (!session_value_opt.has_value() || session_value_opt->empty()) {
    return absl::nullopt;
  }
  const std::string& session_value = session_value_opt.value();
  ...
  const auto& params_data = params.data();
  ...
  for (const auto& param : params_data) {
    if (param.first != param_name_ && !param.second.empty()) {
      estimated_size += param.first.length() + 1 + param.second[0].length() + 1; // "&" + param_name + "=" + value
    }
  }
  ...
  for (const auto& param : params_data) {
    ...
    if (!param.second.empty()) {
      absl::StrAppend(&new_query, "&", param.first, "=", param.second[0]);
    }
  }
