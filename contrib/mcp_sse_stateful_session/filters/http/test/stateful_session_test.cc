...
public:
  void initialize(absl::string_view config, absl::string_view route_config = "") {
    ...
    config_ = std::make_shared<McpSseStatefulSessionConfig>(proto_config, context_.serverFactoryContext());
    ...
    if (!route_config.empty()) {
      ...
      route_config_ =
          std::make_shared<PerRouteMcpSseStatefulSession>(proto_route_config, context_.serverFactoryContext());
...
TEST(EmpytProtoConfigTest, EmpytProtoConfigTest) {
  ...
  McpSseStatefulSessionConfig config(empty_proto_config, context.serverFactoryContext());
