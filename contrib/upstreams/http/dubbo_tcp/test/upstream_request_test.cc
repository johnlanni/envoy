...
TEST_F(TcpUpstreamTest, EmptyConnectConfig) {
  // Clear the connect_config that was set in MockRouterFilterInterface constructor
  mock_router_filter_.callbacks_.route_->route_entry_.connect_config_.reset();
...
TEST_F(TcpUpstreamTest, ConnectConfig) {
  // Ensure connect_config is set (it's already set in MockRouterFilterInterface constructor)
  EXPECT_TRUE(mock_router_filter_.callbacks_.route_->route_entry_.connect_config_.has_value());
