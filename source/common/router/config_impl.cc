...
RouteEntryImplBase::RouteEntryImplBase(const CommonVirtualHostSharedPtr& vhost,
  ...
  if (route.route().has_weighted_clusters()) {
    auto weighted_cluster_plugin = std::make_shared<WeightedClusterSpecifierPlugin>(
        route.route().weighted_clusters(), metadata_match_criteria_.get(), route_name_,
        factory_context, creation_status);
    RETURN_ONLY_IF_NOT_OK_REF(creation_status);
#if defined(HIGRESS)
    // Set up fallback cluster specifier plugin if configured in weighted_clusters
    if (route.route().weighted_clusters().has_inline_cluster_specifier_plugin()) {
      auto fallback_plugin_or_error = getClusterSpecifierPluginByTheProto(
          route.route().weighted_clusters().inline_cluster_specifier_plugin(), validator,
          factory_context);
      if (fallback_plugin_or_error.ok()) {
        weighted_cluster_plugin->setFallbackClusterSpecifierPlugin(
            std::move(fallback_plugin_or_error.value()));
      }
    } else if (!route.route().weighted_clusters().cluster_specifier_plugin().empty()) {
      auto fallback_plugin_or_error = vhost_->globalRouteConfig().clusterSpecifierPlugin(
...
#endif
    cluster_specifier_plugin_ = std::move(weighted_cluster_plugin);
